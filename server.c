...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if (token[0] == NULL || strcmp(token[0], "GET") != 0)
            {
            ...
            // devoid of double-quote
            else if (strchr(token[1], '\"') != NULL)
            {
                // reply Bad Request
                error(400);
                continue;
            ...
            if (token[2] == NULL || strcmp("HTTP/1.1", token[2]) != 0)
            {
            ...
            char* queryInd = strchr(token[1], '?');
            char* eosInd = strchr(token[1], '\0');
            int queryLen = 0;
            
            if (eosInd - queryInd < 1)
                queryLen = 1;
            else
                queryLen = eosInd - queryInd;
            
            char query[queryLen];
            
            if (queryLen == 1)
                memcpy(query, "", queryLen);
            
            else 
                memcpy(query, queryInd + 1, queryLen); 

            // TODO: concatenate root and absolute-path
            
            // absolute path (without query)
            queryInd = strchrnul(token[1], '?');
            char absolutePath[queryInd - token[1] + 1];
            memcpy(absolutePath, token[1], queryInd - token[1]);
            absolutePath[queryInd - token[1]] = '\0';
